...
[Flutter SDK: Clean up todos](https://iterable.atlassian.net/browse/MOB-5139)
...
4. Within the `example/ios` folder, run `pod install`
    - If on Apple M1 processor, first run `sudo arch -x86_64 gem install ffi`. Then run `arch -x86_64 pod install`
5. Open the root folder in VS Code
6. Create a copy of `/example/lib/.env.example`. Rename it `.env` with: 
	- Your own Mobile API Key. You can create it [here](https://app.iterable.com/settings/apiKeys). Make sure the type is `Mobile` and have `JWT Authentication` checked.
	- your JWT token that you manually create on [jwt.io](https://jwt.io/#debugger-io)
      - format the payload like:
        ```
          {
            "exp": "1699575362", // example unix timestamp, usually a year from the iat timestamp
            "email": "some-email@iterable.com", // your email
            "iat": 1668039362 // example unix timestamp of when the JWT token was created
          }
        ```

        - Below the payload in the `VERIFY SIGNATURE` editor, where an editable box reads `your-256-bit-secret`, place your secret from when you generated the Mobile API Key.
7. From the root of the `example/` folder, run ` flutter run`
